...
For active, uninstalled PC entries owned by this plugin, the game menu offers one product-page handoff to the user's preferred app (Xbox app by default, or Microsoft Store). `GamePassProductPages` validates the 12-character Microsoft product ID before building either URI. Opening a page does not prove entitlement, start or verify a download, or mark the Playnite record installed, so this remains a menu action rather than a Playnite install controller.
