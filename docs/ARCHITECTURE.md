...
Each provider/market/language combination has a separate JSON envelope in Playnite's plugin data directory. It contains the fetch timestamp, product IDs, normalized active games with all plan/platform/generation memberships, removed-game history, and diagnostics. The v1.3 schema upgrade invalidates older envelopes for one full refresh. Switching plan, platform, or generation then reuses the same cache.
